...
from scrapers import Authenticator, CourseScraper, EventScraper, NotificationScraper, CalendarScraper
...
def main():
    ...
    print(f"Data saved to {Config.DATA_FILE}. Found {len(data['courses'])} courses, {len(data['events'])} events, {len(data['notifications'])} notifications.")

    # Post to API if enabled
    if Config.API_ENABLED:
        from api_client import APIClient
        api_client = APIClient(Config.API_URL)
        api_client.post_data(data)
    else:
        print("API posting is disabled. Set API_ENABLED=true in your environment to enable.")
